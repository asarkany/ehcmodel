# Author: Kyle Kastner (and modified by Andras Sarkany and Zoltan Milacski)
# License: BSD 3-clause
# Original: https://github.com/kastnerkyle/SciPy2015
...
def minibatch_indices(itr, minibatch_size, shuffle=True, random_state=None,es={}):
    indices = np.arange(len(itr))
    if shuffle == True:
        random_state.shuffle(indices)
    elif shuffle == 'sort_by_angles':
        assert(minibatch_size % es['num_angles'] == 0)
        num_places = minibatch_size / es['num_angles']
        indices = indices.reshape((es['num_angles'],-1))
        for ang in indices:
            np.random.shuffle(ang)
        indices = indices[:,0:((indices.shape[1] / num_places)*num_places)]
        indices = np.split(indices,np.arange(0,indices.shape[1],num_places),axis=1)
        indices = np.hstack([mb.flatten() for mb in indices])
        indices = indices.astype(int)
    excerpt=[]
    for start_idx in range(0, len(itr), minibatch_size):
        end_idx = min(start_idx + minibatch_size,len(itr))
        if end_idx-start_idx==minibatch_size:
            excerpt.append(indices[start_idx:end_idx])
    return excerpt
...
def make_minibatch(arg, indices):
    return [arg[indices,:]]
...
def iterate_function(func, list_of_minibatch_args, minibatch_size,
                     ...
                     shuffle=False, random_state=None,graph = None,pre_epoch_funcs=[],es={}):
    """
    ...
    for e in range(1+n_epochs):
        if e>0:
            for pre_epoch_func in pre_epoch_funcs:
                pre_epoch_func()
            results = defaultdict(list)
            indices = minibatch_indices(list_of_minibatch_args[0], minibatch_size, shuffle, random_state,es)
            for minibatch in indices:
                minibatch_args = []
                for n, arg in enumerate(list_of_minibatch_args):
                    minibatch_args += list_of_minibatch_functions[n](arg, minibatch)
            ...
            if graph != None:
                for k, p in graph.items():
                    if k[:2] == "__":
                        ...
                        # non-grad variables
                        epoch_results[k].append(fetch_from_graph([k], graph)[0].get_value())
        if status_func is not None:
            status_break = status_func(e, epoch_results, status_points)
            if status_break:
                break
...
def add_updates_to_graph(list_of_updates,list_of_names,graph,strict=True):
    assert len(list_of_updates) == len(list_of_names)
    
    for update, name in zip(list_of_updates, list_of_names):
        if name in graph.keys() and strict:
            raise ValueError("Name %s already found in graph!" % name)
        graph['___'+name] = update
        
def add_epoch_counter(graph):
    add_arrays_to_graph([0],['_n_epoch'],graph)
    n_epoch, = fetch_from_graph(['_n_epoch'], graph)
    add_updates_to_graph([(n_epoch,n_epoch+1)],['n_epoch_inc'],graph)
...
def get_pre_epoch_funcs(graph):
    funcs =[]
    for k, p in graph.items():
        if k[:3] == "___":
            # skip private tags
            funcs.append(theano.function([],[],updates=[graph[k]]))
    return funcs
...
def relu(X):
    return tensor.nnet.relu(X)#X * (X > 0)
    
def leaky_relu(X):
    return tensor.nnet.relu(X,0.01)
...
def nonlinearity_layer(list_of_inputs, graph, name, proj_dim=None, random_state=None,
                 strict=True, init_func=np_tanh_fan,func=linear):
    conc_input = concatenate(list_of_inputs, name, axis=-1)
    output = conc_input
    if func is not None:
        final = func(output)
...
def leaky_relu_layer(list_of_inputs, graph, name, proj_dim=None, random_state=None,
               strict=True, init_func=np_tanh_fan):
    return projection_layer(
        list_of_inputs=list_of_inputs, graph=graph, name=name,
        proj_dim=proj_dim, random_state=random_state,
        strict=strict, init_func=init_func, func=leaky_relu)
